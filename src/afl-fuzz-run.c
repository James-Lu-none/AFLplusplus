      ...
      if (st.st_size && st.st_size <= MAX_FILE) {
        ...
        if (likely(next_min_accept < max_start_id ||
          ...
          if (afl->shm.dfg_map) {
            ...
            if (synced_prox_score <= afl->avg_prox_score) {
              if (unlikely(afl->debug)) {
                DEBUGF("Skipping synced seed %s with current average proximity score: %llu <= %llu\n",
                       namelist[o]->d_name, synced_prox_score, afl->avg_prox_score);
              }
              munmap(orig_mem, st.st_size);
              close(fd);
              continue;
